...
def multipladditionoutputting(mode, parameter1mode, parameter2mode): #multipladditionoutputting, your catch-all function for multiplication, addition and outputting
	global codes, pointerincrement, pointer, relativebase
	...
	#print(parameter2mode)
	if parameter1mode == "0":
		parameter1 = safe_list_get(codes, parameter1, False, 0)
	if parameter2mode == "0":
		parameter2 = safe_list_get(codes, parameter2, False, 0)
	if parameter1mode == "2":
		parameter1 = safe_list_get(codes, parameter1+relativebase, False, 0)
	if parameter2mode == "2":
		parameter2 = safe_list_get(codes, parameter2+relativebase, False, 0)
	print(parameter1)
	print(parameter2)
	...
	if mode == "2":
		safe_list_get(codes, codes[pointer+3], True, multipliednumber)
		pointerincrement = 4
	elif mode == "1":
		safe_list_get(codes, codes[pointer+3], True, addednumber)
		pointerincrement = 4
		...
		if parameter1 < parameter2:
			safe_list_get(codes, codes[pointer+3], True, 1)
		else:
			safe_list_get(codes, codes[pointer+3], True, 0) 
		pointerincrement = 4
	elif mode == "8":
		if parameter1 == parameter2:
			safe_list_get(codes, codes[pointer+3], True, 1)
...
def safe_list_get(l, idx, edit, value): 
	global codes
	try:
		if edit:
			l[idx] = value
...
while True:
	...
	if code[-1] == "3":
		if safe_list_get(code, -3, False, 0) == "2":
			codes[codes[pointer+1]+relativebase] = inputvariable
			print(inputvariable)
			print(relativebase)
			print(codes[pointer+1])
			print(codes[pointer+1]+relativebase)
			print(codes[1000])
		else:
			codes[codes[pointer+1]] = inputvariable #thing at second argument becomes input variable
		pointerincrement = 2
	
	if int(code[-1]) in range(1, 10) and int(code[-1]) != 3:
		multipladditionoutputting(code[-1], safe_list_get(code, -3, False, 0), safe_list_get(code, -4, False, 0))
