#!/usr/bin/python3
#make very large array
#give each wire own signature
#make sure they don't cross at start

#KICK ZEV'S ASS!!!
#solve problem (MOST IMPORTANT!!!)
...
crosses = []
lowestdistance = 1111111111111111111
def crossreader(cross):
	global lowestdistance
	distance = grid[str(cross)]
...
for instruction in wire2:
	...
	#like a case statement, but worse
	if direction == "R":
		for i in range(0, amount):
			farnesscounter += 1
			pos2[0] += 1
			if grid[str(pos2)] != 0:
				grid[str(pos2)] += farnesscounter
				crossreader(pos2)
